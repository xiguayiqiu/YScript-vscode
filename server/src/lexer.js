...
function tokenize(source) {
    ...
    const push = (type, value, start, end, l, c) => {
        if (type !== 'whitespace')
            tokens.push({ type, value, start, end, line: l, col: c });
    };
    while (i < len) {
        const ch = source[i];
        const start = i;
        ...
        if (/[0-9]/.test(ch) || (ch === '.' && /[0-9]/.test(source[i + 1] || ''))) {
            ...
            if (source[j] === '0' && (source[j + 1] === 'x' || source[j + 1] === 'X')) {
                j += 2;
                while (j < len && /[0-9a-fA-F_]/.test(source[j]))
                    j++;
            }
            else if (source[j] === '0' && (source[j + 1] === 'b' || source[j + 1] === 'B')) {
                j += 2;
                while (j < len && /[01_]/.test(source[j]))
                    j++;
            }
            else if (source[j] === '0' && (source[j + 1] === 'o' || source[j + 1] === 'O')) {
                j += 2;
                while (j < len && /[0-7_]/.test(source[j]))
                    j++;
            }
            else {
                while (j < len && /[0-9_]/.test(source[j]))
                    j++;
                if (source[j] === '.') {
                    j++;
                    while (j < len && /[0-9_]/.test(source[j]))
        ...
        if (/[_a-zA-Z]/.test(ch)) {
            let j = i;
            while (j < len && /[_a-zA-Z0-9]/.test(source[j]))
                j++;
...
function analyze(tokens, source) {
    const diagnostics = [];
    // 括号配对
    const stack = [];
    const pairs = { '(': ')', '[': ']', '{': '}' };
...
function computeLineMap(source) {
    const lineStarts = [0];
...
exports._internal = { computeLineMap };
/**
 ...
 */
function formatSource(text, tabSize) {
    ...
    const lineTokens = Array.from({ length: lines.length }, () => []);
    ...
    // 跨行 token 的续行标记
    const inCommentCont = new Array(lines.length).fill(false);
    const inTextCont = new Array(lines.length).fill(false);
    for (const t of tokens) {
        if (t.type !== 'comment' && t.type !== 'string' && t.type !== 'shell' && t.type !== 'bytes')
            continue;
 ...
 */
function addSpacing(line, lineStart, toks) {
    const edits = [];
    for (const t of toks) {
    ...
    for (const e of edits) {
        if (e.pos === lastPos)
            continue; // 同一位置只插入一次，避免 `} else  {` 双空格
        lastPos = e.pos;
...
function lastTokenEndingAt(toks, lineStart, pos) {
    let found = null;
    for (const t of toks) {
        const le = t.end - lineStart;
        if (le <= pos)
