...
export type CompletionKind = 'keyword' | 'type' | 'function' | 'method' | 'variable' | 'constant' | 'field' | 'snippet' | 'class' | 'struct' | 'enum' | 'interface' | 'module' | 'property';
 ...
 */
export const TYPE_MEMBERS: Record<string, CompletionItemMeta[]> = {
  string: [
 ...
 */
export const TYPE_INFERENCE_RULES: Array<{ regex: RegExp; type: string }> = [
...
export function buildAllCompletions(): CompletionItemMeta[] {
  const seen = new Map<string, CompletionItemMeta>();
  for (const m of [...KEYWORDS, ...TYPES, ...BUILTIN_FUNCTIONS, ...SNIPPETS]) {
    seen.set(m.label, m);
  }
