...
export const KEYWORDS: CompletionItemMeta[] = [
  ...
  { label: 'warp', kind: 'keyword', detail: '启动并发线程（await 取回结果）', insertText: 'let ${1:h} = warp {\n\t${0}\n}\n${1:h}.await()', sortKey: 10 },
  ...
  { label: 'match', kind: 'keyword', detail: '模式匹配（支持多值/范围）', insertText: 'match ${1:val} {\n\t${2:pattern} => { ${3:body} }\n\t_ => { ${0:default} }\n}', sortKey: 10 },
...
export const BUILTIN_FUNCTIONS: CompletionItemMeta[] = [
  ...
  { label: 'http.Headers', kind: 'builtin', detail: '获取HTTP响应头(大小写不敏感)' },
  { label: 'http.Body', kind: 'builtin', detail: '获取HTTP响应体' },
  { label: 'http.Raw', kind: 'builtin', detail: '完整原始HTTP响应(类似Burp Raw)' },
  ...
  { label: 'sync.select', kind: 'builtin', detail: '通道选择（无default阻塞）', insertText: 'sync.select(sync.case(${1:ch}, ${2:fn}))' },
  { label: 'sync.case', kind: 'builtin', detail: 'select 分支: 通道+回调' },
  { label: 'sync.default', kind: 'builtin', detail: 'select 非阻塞默认分支' },
  ...
  { label: 'time.new_timer', kind: 'builtin', detail: '一次性计时器' },
  { label: 'time.new_ticker', kind: 'builtin', detail: '周期性定时器' },
  ...
  { label: 'sys.cpu_info', kind: 'builtin', detail: 'CPU信息' },
  { label: 'sys.mem_info', kind: 'builtin', detail: '内存信息' },
  { label: 'sys.net_interfaces', kind: 'builtin', detail: '网络接口列表' },
  { label: 'sys.process_list', kind: 'builtin', detail: '进程列表' },
  { label: 'sys.process_info', kind: 'builtin', detail: '进程详情' },
  { label: 'sys.find_process', kind: 'builtin', detail: '按名查找进程' },
  { label: 'sys.kill_process', kind: 'builtin', detail: '终止进程' },
  ...
  { label: 'ffi.call', kind: 'builtin', detail: '调用C函数（真实ABI，0-8参数）' },
  { label: 'ffi.find', kind: 'builtin', detail: '查找符号（配合 ffi.call）' },
  { label: 'ffi.alloc', kind: 'builtin', detail: '分配真实C堆内存' },
  { label: 'ffi.free', kind: 'builtin', detail: '释放 ffi.alloc 内存' },
  { label: 'ffi.read', kind: 'builtin', detail: '从真实内存地址读 n 字节' },
  { label: 'ffi.write', kind: 'builtin', detail: '向真实内存地址写字节' },
  { label: 'ffi.str', kind: 'builtin', detail: '读取真实内存中的 C 字符串' },

  { label: 'cuda.device_count', kind: 'builtin', detail: 'GPU设备数' },
  { label: 'cuda.MD5_Batch', kind: 'builtin', detail: 'GPU批量MD5' },
  { label: 'cuda.SHA256_Batch', kind: 'builtin', detail: 'GPU批量SHA256' },
  { label: 'cuda.NTLM_Batch', kind: 'builtin', detail: 'GPU批量NTLM' },
  { label: 'cuda.WPA2_PMK_Batch', kind: 'builtin', detail: 'GPU批量WPA2 PMK' },
  { label: 'regex.replace_fn', kind: 'builtin', detail: '正则回调替换: (pat, str, fn)' },
  { label: 'array.Map', kind: 'builtin', detail: '列表变换（支持用户函数）' },
  { label: 'array.Filter', kind: 'builtin', detail: '列表过滤（支持用户函数）' },
  { label: 'array.Reduce', kind: 'builtin', detail: '列表聚合（支持用户函数）' },
  { label: 'iter.map', kind: 'builtin', detail: '迭代变换' },
  { label: 'iter.filter', kind: 'builtin', detail: '迭代过滤' },
  { label: 'iter.take', kind: 'builtin', detail: '取前n个' },
  { label: 'iter.chain', kind: 'builtin', detail: '串联多个迭代' },
  { label: 'time.new_timer', kind: 'builtin', detail: '一次性计时器(ms): wait/ready/stop' },
  { label: 'time.new_ticker', kind: 'builtin', detail: '周期性定时器(ms): wait/stop' },
  { label: 'raw.PcapOpen', kind: 'builtin', detail: '打开设备实时抓包' },
  { label: 'raw.PcapNext', kind: 'builtin', detail: '读取下一包（超时返回nil）' },
  { label: 'raw.PcapFilter', kind: 'builtin', detail: '设置BPF过滤器' },
  { label: 'raw.PcapClose', kind: 'builtin', detail: '关闭抓包句柄' },
  { label: 'raw.PcapReadFile', kind: 'builtin', detail: '读取.pcap文件' },
  { label: 'raw.PcapWriteFile', kind: 'builtin', detail: '写入.pcap文件' },
];
...
export const TYPE_MEMBERS: CompletionItemMeta[] = [
  ...
  { of: 'dict', label: 'headers', kind: 'method', detail: 'HTTP响应头(仅HTTP响应有效)' },
  { of: 'dict', label: 'body', kind: 'method', detail: 'HTTP响应体(仅HTTP响应有效)' },
  { of: 'dict', label: 'raw', kind: 'method', detail: '完整原始HTTP响应(仅HTTP响应有效)' },
  ...
  // === warp handle members ===
  { of: 'warp', label: 'await', kind: 'method', detail: '等待结束并返回结果' },
  { of: 'warp', label: 'detach', kind: 'method', detail: '分离线程（主线程退出不等待）' },
  { of: 'warp', label: 'hush', kind: 'method', detail: '请求停止' },
  { of: 'warp', label: 'snap', kind: 'method', detail: '强制停止' },
  { of: 'warp', label: 'living', kind: 'method', detail: '是否存活' },
  { of: 'warp', label: 'link', kind: 'method', detail: '连锁：子线程停止时自身停止' },
  { of: 'warp', label: 'unlink', kind: 'method', detail: '取消连锁' },

  // === WaitGroup members ===
  { of: 'sync.WaitGroup', label: 'add', kind: 'method', detail: '增加计数 n' },
  { of: 'sync.WaitGroup', label: 'done', kind: 'method', detail: '完成一个（计数-1）' },
  { of: 'sync.WaitGroup', label: 'wait', kind: 'method', detail: '阻塞直到计数归零' },
];
...
export const SNIPPETS: CompletionItemMeta[] = [
  ...
  { label: 'method', kind: 'snippet', detail: 'struct 方法（this）', insertText: 'func this.${1:name}(${2:args}) {\n\t${0}\n}\n', sortKey: 5 },
  { label: 'ternary', kind: 'snippet', detail: '三元表达式', insertText: '${1:cond} ? ${2:a} : ${3:b}', sortKey: 5 },
  { label: 'range', kind: 'snippet', detail: '区间遍历 0..n', insertText: 'for ${1:i} in ${2:0}..${3:n} {\n\t${0}\n}\n', sortKey: 5 },
  { label: 'multiret', kind: 'snippet', detail: '多返回值', insertText: 'func ${1:name}() {\n\treturn ${2:a}, ${3:b}\n}\nlet ${4:x}, ${5:y} = ${1:name}()\n', sortKey: 5 },
